...
- test
    ...
    - then, use this method to test a real HTTPS website. When the victim and the proxy do handshake, they use our own CA and server certificate generated by this CA. When the proxy does handshake with the real server, we verify the server's certificate using certificates of real CAs in `/etc/ssl/certs`
    - I successfully visited `www.ed.ac.uk`, however, when I try to visit the login page at `www.ease.ed.ac.uk`, there is a 404 error. For simplicity, I use an alternative approach, as most login will send a POST request, with the data in the request body, we use HTTP Header Live to construct a POST request to `www.ed.ac.uk`, the body is of JSON format `{ 'username':'user', 'password':'dees' }`
        ```
        pprint.pprint("Request: {}".format(request))
		header, _, body = request.partition(b'\r\n\r\n')
		print("Body:", body) # print the content in the request body
        ```
        ```
        Body: b"{\xc2\xa0\xc2\xa0\xc2\xa0 'username':'user',\xc2\xa0\xc2\xa0\xc2\xa0 'password':'dees'}"
        ```
    - if we use Wireshark to capture packets, we will not be able to find the password, as data through TLS is encrypted, the proxy can see the data, because it's impersonating the server and does handshake with the client using its own certificate
