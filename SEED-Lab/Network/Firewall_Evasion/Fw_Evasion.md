...
- then, we also need to configure some routing rules
    ...
    - on A, send the website's response packets through the tunnel `ip route add 192.168.20.5 via 192.168.53.99 dev tun0` (in task `3.1`, we didn't add routing rule to VPN server (B), because in that case, internal network can directly send packets to external network via the router. I inspected the packets captured by `Wireshark`, and found a TCP packet that contains the result of the command I typed (it was not a SSH packet), its source MAC address was the router's MAC address, and destination was A1, this means the response packet was indeed routed by the router, not via the VPN tunnel. If we want the traffics in both directions go through the VPN tunnel, we will need to add a routing rule in B in task `3.1`)
